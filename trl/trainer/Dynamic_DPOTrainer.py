...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.base_beta = self.args.beta  # Set the base beta
    ...
    def tokenize_row(features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        weight  = features.get("weight", 1.0)  # defalt weight = 1.0
    ...
    def dpo_loss(
        ...
        # Adjust beta dynamically using rank
        weighted_beta = self.base_beta * weight.to(device)  # rank 값을 beta에 곱해 동적으로 조정
        ...
        if self.loss_type == "sigmoid":
            losses = (
                -F.logsigmoid(weighted_beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-weighted_beta * logits) * self.label_smoothing
            )
            ...
            losses = (weighted_beta * logits).sigmoid() * (
                F.logsigmoid(weighted_beta * logits) - math.log(1 - self.label_smoothing)
            ) + (-weighted_beta * logits).sigmoid() * (F.logsigmoid(-weighted_beta * logits) - math.log(self.label_smoothing))

        elif self.loss_type == "hinge":
            losses = torch.relu(1 - weighted_beta * logits)

        elif self.loss_type == "ipo":
            # eqn (17) of the paper where beta is the regularization parameter for the IPO loss, denoted by tau in the paper.
            losses = (logits - 1 / (2 * weighted_beta)) ** 2

        elif self.loss_type == "bco_pair":
            chosen_logratios = chosen_logps - ref_chosen_logps
            rejected_logratios = rejected_logps - ref_rejected_logps
            chosen_rewards = weighted_beta * chosen_logratios
            rejected_rewards = weighted_beta * rejected_logratios
            rewards = torch.cat((chosen_rewards, rejected_rewards), 0).mean().detach()
            self.running.update(rewards)
            delta = self.running.mean
            losses = -F.logsigmoid((weighted_beta * chosen_logratios) - delta) - F.logsigmoid(
                -(weighted_beta * rejected_logratios - delta)
            )
            ...
            losses = (a - 0.5 / weighted_beta) ** 2 + (b + 0.5 / weighted_beta) ** 2

        elif self.loss_type == "nca_pair":
            chosen_rewards = (chosen_logps - ref_chosen_logps) * weighted_beta
            rejected_rewards = (rejected_logps - ref_rejected_logps) * weighted_beta
            losses = (
            ...
            losses = (
                -F.logsigmoid(weighted_beta * delta) * (1 - self.label_smoothing)
                - F.logsigmoid(-weighted_beta * delta) * self.label_smoothing
            )
            ...
            losses_chosen = 1 - F.sigmoid(weighted_beta * chosen_logratios)  # Increase chosen likelihood
            losses_rejected = F.sigmoid(weighted_beta * rejected_logratios)  # Decrease rejected likelihood
            ...
            losses_chosen = F.sigmoid(weighted_beta * chosen_logratios)
            losses_rejected = 1 - F.sigmoid(weighted_beta * (chosen_logratios - rejected_logratios))
            losses = losses_chosen + losses_rejected
        ...
        chosen_rewards = weighted_beta * (chosen_logps.to(device) - ref_chosen_logps.to(device)).detach()
        rejected_rewards = weighted_beta * (rejected_logps.to(device) - ref_rejected_logps.to(device)).detach()
    ...
    def get_batch_loss_metrics(
        ...
        # Extract rank from the batch
        weight = batch["weight"]  # Assume that weights are included in batch
        losses, chosen_rewards, rejected_rewards = self.dpo_loss(
            model_output["chosen_logps"], model_output["rejected_logps"], ref_chosen_logps, ref_rejected_logps, weight
        )
